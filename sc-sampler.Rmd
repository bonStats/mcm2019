...
$$f_{\tilde{\boldsymbol{\theta}}}(\boldsymbol{\theta})~f_{\tilde{\boldsymbol{\omega}}}(\boldsymbol{\omega})~1(\boldsymbol{r}(\boldsymbol{\theta}) \preceq \boldsymbol{\omega})  $$
...
- Scale mixture of normals/uniforms and other latent variable approaches
- Slice sampling
...
**Normal model**: $(\boldsymbol{y} ~\vert~ \boldsymbol{X}, \boldsymbol{\beta},\sigma^2) \sim \text{N}(\boldsymbol{X} \boldsymbol{\beta}, \sigma^2)$.
...
**Complexity**: $\mathcal{O}(\vert J \vert^{3} + p \vert I\vert)$ in each iteration.

--

We can fix $\vert J \vert^{3}$ by selection $\epsilon$ adaptively then complexity is $\mathcal{O}(p^2)$.

---

 - In high dimesions with shrinkage priors, the majority of coefficients are forced to be very close to zero.
...
- Normal regression model with R2-D2 prior
- $n = 100$
